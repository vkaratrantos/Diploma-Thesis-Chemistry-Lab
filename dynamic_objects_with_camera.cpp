...
void setupCollisionObjects(const std::string& frame_id, tf2_ros::Buffer& tf_buffer) {
    ...
    for (int i = 1; i <= 6; ++i) {
        ...
        try {
            ...
            tube_y = t.transform.translation.y + 0.04; // Exactly 4cm behind the marker on the Y axis
            ...
            if (ss >> marker_id) {
                ...
                if (marker_id == 0) {
                    ...
                    try {
                        ...
                        // Target exactly 4cm behind the marker location
                        tx = t.transform.translation.x;
                        ty = t.transform.translation.y + 0.04; 
                        
                        // Set arm target height safely above the table/marker for grabbing
                        tz = t.transform.translation.z + 0.15; 
                    } catch (const tf2::TransformException & ex) {
