...
int main(int argc, char * argv[])
{
    ...
    q_upright.setRPY(0.0, -M_PI/2, M_PI/2); 
    ...
    while (rclcpp::ok()) {
        ...
        // Hardcode the upright baseline to prevent floating point drift
        ocm.orientation.x = q_upright.x();
        ocm.orientation.y = q_upright.y();
        ocm.orientation.z = q_upright.z();
        ocm.orientation.w = q_upright.w();

        // Roll is completely free. Pitch/Yaw get 0.15 rad tolerance.
        ...
        // --- B. PLANNER SETTINGS ---
        // Using RRTConnect to leverage the newly configured Constrained State Space in YAML
        arm_interface.setPlannerId("RRTstarkConfigDefault");
        arm_interface.setGoalPositionTolerance(0.02); 
        arm_interface.setGoalOrientationTolerance(0.4); 
        arm_interface.setNumPlanningAttempts(1); 

        // --- C & D. GOAL SEARCH & ITERATIVE PLANNING (FAIL FAST ARCHITECTURE) ---
        std::vector<double> roll_offsets = {0.0};
        for (int i = 1; i <= 180; i += 1) {
            double angle_in_rads = (i * 1.0) * (M_PI / 180.0);
            roll_offsets.push_back(angle_in_rads);
            roll_offsets.push_back(-angle_in_rads);
        }

        // Refresh state and create planning scene for collision checking
        ...
        auto planning_scene = std::make_shared<planning_scene::PlanningScene>(arm_interface.getRobotModel());
        
        // Define the self-collision callback
        ...
        // Set an aggressive planning timeout. If it can't find a path in 3 seconds, 
        // it fails fast and moves to the next wrist angle.
        arm_interface.setPlanningTime(2.0); 
        ...
        for (double roll : roll_offsets) {
            ...
            // 1. Check if the pose is mathematically valid AND collision-free
            bool found_valid_solution = kinematic_state->setFromIK(joint_model_group, target, 0.2, collision_callback);
            
            if (found_valid_solution) {
                std::cout << "\n>>> Valid & Collision-Free IK found at roll: " 
                          << roll * (180.0 / M_PI) << " degrees. Attempting plan..." << std::endl;
                
                // 2. Attempt to plan the path immediately
                arm_interface.setPoseTarget(target);
                MoveGroupInterface::Plan my_plan;
                
                if (arm_interface.plan(my_plan) == moveit::core::MoveItErrorCode::SUCCESS) {
                    std::cout << ">>> Path found! Executing trajectory..." << std::endl;
                    arm_interface.execute(my_plan);
                    successful_move = true;
                    break; // Mission accomplished. Stop searching!
                } else {
                    std::cout << "[-] Path blocked or timed out. Moving to next roll angle..." << std::endl;
                    // Loop continues automatically to find a better angle.
                }
            }
        }

        if (!successful_move) {
            std::cout << "\n[-] Exhausted all roll angles. Could not find a reachable AND plannable path." << std::endl;
        }
    }
